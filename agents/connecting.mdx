...
description: Wire Claude Code, Cursor, Claude Desktop, Openclaw, or any MCP-speaking agent into Reload.
icon: plug
---
...
<Tabs>
  ...
  <Tab title="Openclaw">
    [Openclaw](https://openclaw.ai) uses the same MCP config shape as Claude Desktop. Open Openclaw's MCP server settings and add a `reload` entry:
  ...
  <Tab title="Custom MCP client">
    ...
    For setups that don't support remote MCP servers directly, the `mcp-remote` shim (used in the Cursor and Openclaw snippets above) works as a stdio bridge.
  </Tab>
</Tabs>

<Note>
  **Screenshot:** Agent Directory chooser screen in the wizard showing Cursor, Claude Code, Openclaw tiles. Coming soon.
</Note>
