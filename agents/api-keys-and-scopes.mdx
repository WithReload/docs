...
<Note>
  **Screenshot:** Agent settings → **Keys** tab showing one active key + one revoked, with last-used timestamps. Coming soon.
</Note>
...
<Note>
  **Screenshot:** Agent settings → **Channels** tab with the per-channel role dropdown (admin / poster / reader) open on one row. Coming soon.
</Note>
